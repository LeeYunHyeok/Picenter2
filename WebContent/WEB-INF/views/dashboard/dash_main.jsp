							...
							function resizeGraph_One(result, status, ansyn) {
								...
								$.each(result, function(key, value){
								    $.each(value, function(key, value){
								        if(key == "regdate") days.push(value);
								        if(key == "type1") type1.push(value);
								        if(key == "type2") type2.push(value);
								        if(key == "type3") type3.push(value);
								        if(key == "type4") type4.push(value);
								        if(key == "type5") type5.push(value);
								        if(key == "type6") type6.push(value);
								    });
								});
								*/
 ...
 * 
 if(result[i].TYPE1 != 0){ 
		...
		$("#"+type+"_name"+(i+1)).prop("href", "javascript:exception('"+result[i].TARGET_ID+"')");
		$("#"+type+"_name"+(i+1)).html(result[i].NAME +"\n"+ result[i].IP);
		var regexp = /\B(?=(\d{3})+(?!\d))/g;  // 천단위 콤마 넣기 
		var type1 = String(result[i].TYPE1).replace(regexp, ',');
		$("#"+type+"_"+(i+1)).html(type1);
	}
